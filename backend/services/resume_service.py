...
class ResumeService:
    ...
    async def generate_resume_coaching(self, resume_text: str, notice_text: str):
        ...
        system_prompt = """
        당신은 전문 커리어 코치이자 채용 데이터 분석가입니다. [채용 공고]의 요구사항과 [지원자 이력서]를 
        동일 직무의 '합격자 데이터베이스'와 정밀 비교하여 벤치마킹 리포트를 생성해주세요.
        
        분석 항목:
        1. 벤치마크 점수 (benchmark_score): 합격자 평균 대비 현재 자소서의 경쟁력 (0~100점)
        2. 누락된 핵심 키워드 (missing_keywords): 합격자 자소서에는 빈번하지만 현재 자소서에는 누락된 단어들
        3. 차별화 포인트 (differentiation): 지원자만이 가진 독특한 강점 분석
        4. 구체적인 문장 수정 예시 (Before/After)
        
        응답은 반드시 아래의 JSON 형식으로만 해주세요:
        {
            "benchmark_score": 점수,
            "summary": "전체적인 전략 요약",
            "missing_keywords": ["키워드1", "키워드2"],
            "differentiation": "본인만의 차별화 지점",
            "suggestions": [
